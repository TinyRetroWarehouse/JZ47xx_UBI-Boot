...
void serial_putb(unsigned int d) {
	unsigned int m;
	for (m = 0x80000000; m != 0x00800000; m >>= 1) {
		serial_putc(m & d ? '1' : '0');
	}
	serial_putc(' ');
	for (m = 0x00800000; m != 0x00008000; m >>= 1) {
		serial_putc(m & d ? '1' : '0');
	}
	serial_putc(' ');
	for (m = 0x00008000; m != 0x00000080; m >>= 1) {
		serial_putc(m & d ? '1' : '0');
	}
	serial_putc(' ');
	for (m = 0x00000080; m != 0x00000000; m >>= 1) {
		serial_putc(m & d ? '1' : '0');
	}
}
