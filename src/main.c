...
static char *kernel_params[] = {
	[PARAM_EXEC] = "linux", [PARAM_LOWMEM] = "mem=0x0000M",
#ifdef USES_HIGHMEM
...
#endif
	[PARAM_CONSOLE_LOCAL] = "console=tty0", [PARAM_LOGO] = "",
#ifdef JZ_SLCD_PANEL
...
#endif
	[PARAM_SEPARATOR] = "--", [PARAM_HWVARIANT] = "hwvariant=" VARIANT,[PARAM_KERNEL_BAK] = "",[PARAM_ROOTFS_BAK] = "",
#if BENCHMARK
...
void c_main(void) {
	...
	if(mmc_inited) {
		if(mmc_load_kernel(MMC_ID, (void *) (KSEG1 + LD_ADDR), alt_kernel, &exec_addr) == 1) {
			set_alt_param();
		}
		
		if(exec_addr) {
#if PASS_ROOTFS_PARAMS
			kernel_params[PARAM_ROOTDEV] = "root=/dev/mmcblk0p1";
...
#warning UBI is currently the only supported NAND file system and it was not selected.
	...
	*/
	...
	/* Boot the kernel */
	((kernel_main) exec_addr)(ARRAY_SIZE(kernel_params), kernel_params, NULL, NULL);
}
