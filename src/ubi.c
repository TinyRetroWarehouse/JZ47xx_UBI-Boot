...
static uint32_t get_kernel_vol_id(uint32_t eb, uint32_t data_page, uint32_t nb_volumes, unsigned int id) {
	unsigned long page = eb * PAGE_PER_BLOCK + data_page;
	struct ubi_vol_tbl_record *records;
	unsigned int nb = div_round_up(nb_volumes * sizeof(*records), PAGE_SIZE);
	unsigned int i;
	...
	for (i = 0; i < nb_volumes; i++) {
		...
		if(!strncmp((const char *) records[i].name, volume_name(id), volume_name_len(id))) {
			return i;
		}
	}
...
static int load_kernel(uint32_t eb_start, uint32_t nb_ebs, uint8_t *ld_addr, void **exec_addr, unsigned int kernel_volume) {
	...
	kernel_vol_id = get_kernel_vol_id(SLIST_FIRST(&eb_list[2])->peb, data_page, 8, kernel_volume);
	...
	for (i = 0; i < loaded[kernel_vol_id]; i++) {
		...
		if(i == 0) {
			...
			ld_addr = process_uimage_header((void *) ld_addr, exec_addr, PAGE_SIZE);
			if(!ld_addr) {
...
int ubi_load_kernel(unsigned char *ld_addr, void **exec_addr, uint32_t vol_id) {
	return load_kernel(UBI_MTD_EB_START, UBI_MTD_NB_EB, ld_addr, exec_addr, vol_id);
}
