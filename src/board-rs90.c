...
#include "include/jz4740-cpm.h"
#include "include/jz4740-emc.h"
#include "include/jz4740-gpio.h"
...
static void pll_init(void) {
	...
	static const uint8_t n2FR[33] = {
		0, 0, 1, 2, 3, 0, 4, 0, 5, 0, 0, 0, 6, 0, 0, 0, 7, 0, 0, 0, 0, 0, 0, 0, 8, 0, 0, 0, 0, 0, 0, 0, 9
	};
	
	cfcr = CPM_CPCCR_CLKOEN | CPM_CPCCR_PCS | ((unsigned int) n2FR[CDIV] << CPM_CPCCR_CDIV_BIT) | ((unsigned int) n2FR[HDIV] << CPM_CPCCR_HDIV_BIT) | ((unsigned int) n2FR[PDIV] << CPM_CPCCR_PDIV_BIT) | ((unsigned int) n2FR[MDIV] << CPM_CPCCR_MDIV_BIT) | ((unsigned int) n2FR[LDIV] << CPM_CPCCR_LDIV_BIT);
...
unsigned int get_memory_size(void) {
	return 1 << (SDRAM_ROW + SDRAM_COL + (2 - SDRAM_BW16) + (2 - SDRAM_BANK4) + 1);
}
...
void nand_init(void) {
	REG32(EMC_SMCR1) = (EMC_TAS << EMC_SMCR_TAS_BIT) | (EMC_TAH << EMC_SMCR_TAH_BIT) | (EMC_TBP << EMC_SMCR_TBP_BIT) | (EMC_TAW << EMC_SMCR_TAW_BIT) | (EMC_STRV << EMC_SMCR_STRV_BIT);
}
