 ...
 */
static int cluster_span(unsigned int id, uint32_t cluster, uint32_t *next, uint32_t *count) {
	uint32_t sector[FAT_BLOCK_SIZE >> 2];
 ...
 */
static void *load_cluster_chain(unsigned int id, uint32_t cluster, void *ld_addr, void **exec_addr) {
...
static struct dir_entry *find_file(struct dir_entry *first, struct dir_entry *end, const char *name) {
	...
	for (i = 0; i < 4; i++) {
		...
		if(entry) {
			...
			*exec_addr = ld_addr;
			...
			if(load_cluster_chain(id, entry->starthi << 16 | entry->start, ld_addr, (kernel & 1) ? NULL : exec_addr)) {
				return kernel >> 1;
			}
			err = -1;
		}
